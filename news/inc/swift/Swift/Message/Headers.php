  ...
  public function getEncoded($name)
  {
    ...
    foreach ($encoded_value as $key => $row)
    {
      ...
      if (false !== $p = strpos($encoded_value[$key], $this->LE))
      {
        ...
        //~ $encoded_value[$key] = preg_replace("/<([^>]+)>/e", $cb, $encoded_value[$key]);#php7 Warning: preg_replace(): The /e modifier is no longer supported, use preg_replace_callback instead in plugins/gutuma/news/inc/swift/Swift/Message/Headers.php on line 424
        $encoded_value[$key] = preg_replace_callback('~<([^>]+)>~',
          function ($matches) {
            return str_replace($this->LE, '', '<'.$matches[0].'>');
          },
          $encoded_value[$key]
        );#php7 fixWarning: preg_replace(): The /e modifier is no longer supported, use preg_replace_callback instead in plugins/gutuma/news/inc/swift/Swift/Message/Headers.php on line 424
      }
