	...
	foreach($list_ids as $list_id){// For each list we need to load it with all addresses
		...
		if($list){//!= false (object)
			...
			if($list->is_private())// Don't allow subscriptions to private lists)
				...
				if($hash AND $ok = $listi->contains($address, TRUE, $hash)) {//Second time with good key (real add)
					$lastTime = TRUE;
					if($subscribe){
					...
					if($res){//send text & link to validate 1st time of (un)subscrition
						...
						$keytext .= "\n\n".t('To approve your % for list "%" visit: ',array(strtolower($what), $list->get_name()))."\n".$subscribe_url.'addr='.$address.'&list='.$list->get_id().'&action='.($subscribe ?'':'un').'subscribe&k='.$keycode."\n\n".t('Use this link before %, after this date, you need a request new valid link.',array(date(t('Y-m-d H:i'),($listi->timeAddress + $valtime))))."\n".str_repeat("-",72)."\n\n";
					}
...
//	Work out if we need to send any emails now, and if so create a sender
	...
	if($mailer->init()){
		...
		if ($sendkey){//send mail with security link and user notice : 1st time or unsub
			...
			$text .= "\n".str_repeat('-',72).$keytext."\n\n".t('To change your subscription'.($plural?'s':'').' visit: ')."\n".$subscribe_url.'addr='.$address.((count($succ_list_names) == 1) ? "&list=".$list_id : "")."\n\n";
...
//echo '<pre>subject : '.$subject.PHP_EOL.$text.'</pre>';#dbg
			...
			return gu_success(t('Thank you for your').' &quot;'.$action.'&quot; '.$address.', '.t('first step successful!').'<br />'.t('You will receive one email to validate your % for following list'.($plural?'s':'').': <i>%</i>', array($action,implode('</i>, <i>', $succ_list_names))).'<br /><br /><i>'.t('Remember to <b>check in spam mails</b> if the message are not found in your inbox').'.</i>');
...
//			Send welcome / goodbye message
			if (($subscribe && gu_config::get('list_send_welcome')) || (!$subscribe && gu_config::get('list_send_goodbye'))){
				...
				$text .= t('To change your subscription'.($plural?'s':'').' visit: ')."\n".$subscribe_url.'addr='.$address.((count($succ_list_names) == 1) ? "&list=".$list_id : "")."\n\n";
				$text .= t('Please do not reply to this message. Thank you.');
...
//echo '<pre>'.$subject.PHP_EOL.$text.'</pre>';
	...
	return gu_success(t('You have been % list'.($plural?'s':'').': <i>%</i>', array(str_replace('  ',' ',$action),implode('</i>, <i>', $succ_list_names))).($sendok?'<br /><br /><i>'.t('Remember to <b>check in spam mails</b> if the message are not found in your inbox').'.</i>':''));
}