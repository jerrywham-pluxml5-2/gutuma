...
class EasySwift
{
  ...
  public function __construct(Swift_Connection $connection, $domain=null)
  {
    try {
      ...
      $this->failed = true;
      $this->setError(t("The connection failed to start.  An exception was thrown:<br />") . $e->getMessage());
    }
    $this->newMessage();
  ...
  public function flushParts()
  {
    ...
    foreach ($this->partIds as $id)
    {
      try {
        ...
        $success = false;
        $this->setError(t("A MIME part failed to detach due to the error:<br />") . $e->getMessage());
      }
    }
    $this->partIds = array();
    return $success;
  ...
  public function flushAttachments()
  {
    ...
    foreach ($this->attachmentIds as $id)
    {
      try {
        ...
        $success = false;
        $this->setError(t("An attachment failed to detach due to the error:<br />") . $e->getMessage());
      }
    }
    $this->attachmentIds = array();
    return $success;
  ...
  public function connect()
  {
    if (!$this->isConnected())
    {
      try {
        ...
        $this->failed = true;
        $this->setError(t("Swift failed to run the connection process:<br />") . $e->getMessage());
      }
    }
    return false;
  }
  ...
  public function close()
  {
    if ($this->isConnected())
    {
      try {
        ...
        $this->setError(t("Disconnect failed:<br />") . $e->getMessage());
      }
    }
    return false;
  }
  ...
  public function command($command)
  {
    ...
    try {
      ...
      $this->setError(t("Command failed:<br />") . $e->getMessage());
      return false;
    }
  }
  /**
  ...
  public function loadAuthenticator(Swift_Authenticator $auth)
  {
    ...
    else throw new Exception(t("SMTP authentication cannot be used with connection class '%'. Swift_Connection_SMTP is needed",array(get_class($this->connection))));
  }
  /**
  ...
  public function authenticate($username, $password)
  {
    if (method_exists($this->swift->connection, "runAuthenticators"))
    {
      try {
        ...
        $this->setError(t("Authentication failed:<br />") . $e->getMessage());
        return false;
      }
    }
    else throw new Exception(t("SMTP authentication cannot be used with connection class '%'. Swift_Connection_SMTP is needed",array(get_class($this->connection))));
  ...
  public function setCharset($charset)
  {
    try {
      ...
      $this->setError(t("Unable to set the message charset:<br />") . $e->getMessage());
      return false;
    }
  }
  /**
  ...
  public function addPart($body, $type="text/plain", $encoding=null, $charset=null)
  {
    if ($body instanceof Swift_Message_Mime)
    {
      try {
        ...
        $this->setError(t("A MIME part failed to attach:<br />") . $e->getMessage());
        return false;
      }
    }
    else
  ...
  public function addAttachment($data, $filename=null, $type="application/octet-stream", $encoding=null)
  {
    if ($data instanceof Swift_Message_Mime)
    {
      try {
        ...
        $this->setError(t("An attachment failed to attach:<br />") . $e->getMessage());
        return false;
      }
    }
    else
      ...
      foreach ($attribute_pairs as $pair)
      {
        ...
        if (preg_match("~^(.*?)=(\"?)(.*?)\\2\\s*\$~", $pair, $matches))
        {
          try {
            ...
            $this->setError(t("There was a problem parsing or setting a header attribute:<br />") . $e->getMessage());
  ...
  public function send($recipients, $from, $subject, $body=null)
  {
    ...
    try {
      ...
      $this->setError(t("Sending failed:<br />") . $e->getMessage());
      return false;
    }
  }
}
